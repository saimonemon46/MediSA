...
function reportDoctorLink(r) {
  ...
  const conversationText = buildConversationContext()
    ...
    .join(' ');
  const reportText = [r.reasoning, r.guidance, r.explanation, conversationText].filter(Boolean).join(' ').slice(0, 900);
...
async function startSession(symptomText) {
  ...
  try {
    ...
    window._qIdx = 0;
    addMessage("Thank you. I have a few follow-up questions:\n\n" + window._questions[0], 'ai');
  }
...
async function generateReport() {
  ...
  try {
    ...
    currentReport = {
      ...
      reasoning: 'The symptoms you described — including the onset, severity, and pattern — are consistent with a viral upper respiratory infection. This is grounded in the retrieved medical knowledge base which links these symptom patterns to common viral infections.',
      recommended_specialist: 'General Physician',
      guidance: 'Rest, stay hydrated, and monitor your temperature. If fever exceeds 39°C or symptoms worsen significantly within 48 hours, seek immediate medical attention. You may take paracetamol for fever and pain relief.',
...
function displayReportReady(report) {
  ...
  const urgencyClass = { high:'badge-high', medium:'badge-medium', low:'badge-low' }[report.urgency] || 'badge-medium';
...
function renderReportPanel(r) {
  ...
  `;
  const doctorLink = document.getElementById('reportContent').querySelector('a[href="doctors.html"]');
  if (doctorLink) {
