...
const DEMO_REPORTS = [
  { id:1, possible_condition:'Viral Upper Respiratory Infection', urgency:'medium', recommended_specialist:'Medicine Specialist', reasoning:'Symptoms including fever, sore throat, and nasal congestion match viral URI patterns in the medical knowledge base.', guidance:'Rest, hydration, paracetamol for fever. Seek care if symptoms worsen after 5 days.', symptoms_listed:['Fever 38.2 C','Sore throat','Runny nose','Fatigue'], created_at:'2026-03-12T10:30:00' },
  { id:2, possible_condition:'Migraine', urgency:'medium', recommended_specialist:'Neurologist', reasoning:'Unilateral throbbing headache with photophobia and nausea is a classic migraine pattern supported by the symptom knowledge base.', guidance:'Rest in a dark quiet room. Take prescribed migraine medication. Track triggers.', symptoms_listed:['Severe headache','Nausea','Light sensitivity','Throbbing sensation'], created_at:'2026-03-05T14:20:00' },
  { id:3, possible_condition:'Allergic Rhinitis', urgency:'low', recommended_specialist:'Medicine Specialist', reasoning:'Persistent sneezing, watery eyes, and nasal congestion without fever strongly suggests allergic rhinitis per the medical knowledge base.', guidance:'Antihistamines recommended. Identify and avoid allergen triggers.', symptoms_listed:['Sneezing','Watery eyes','Nasal congestion','Itchy nose'], created_at:'2026-02-20T09:15:00' },
];

function urgencyClass(u) {
  return { high:'badge-high', medium:'badge-medium', low:'badge-low' }[u] || 'badge-medium';
}

function reportDoctorLink(r) {
  const params = new URLSearchParams();
  if (r.recommended_specialist) params.set('specialization', r.recommended_specialist);
  if (Array.isArray(r.symptoms_listed) && r.symptoms_listed.length) params.set('symptom', r.symptoms_listed.join(', '));
  if (r.possible_condition) params.set('condition', r.possible_condition);
  if (r.urgency) params.set('urgency', r.urgency);
  const reportText = [r.reasoning, r.guidance, r.explanation].filter(Boolean).join(' ').slice(0, 700);
...
function renderReportsList(reports) {
  ...
  if (!reports.length) {
    el.innerHTML = '<div class="card"><p class="text-muted" style="text-align:center;padding:40px 0">No reports yet. <a href="symptom-chat.html" class="text-teal">Start a symptom check -></a></p></div>';
    return;
  }
      ...
      <div class="report-body">
        <div style="display:flex;gap:32px;flex-wrap:wrap">
          ...
          <div>
            ...
            <div>${(r.symptoms_listed || []).slice(0, 3).map(s => escHtml(s)).join(', ')}</div>
          </div>
        </div>
...
function openReport(id) {
  ...
  const symptoms = (r.symptoms_listed || []).map(s => `<li>${escHtml(s)}</li>`).join('');
...
async function loadReports() {
  const u = getUser();
  try {
    const res = await fetch(PHP_BASE + '/api/reports.php?user_id=' + (u?.id || 1));
